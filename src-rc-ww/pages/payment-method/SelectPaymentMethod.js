...
import { onBoardingShowed, createCustomer, createSetupIntent, updateUserOnBoardingStatus, handleUserDetails, handlePaymentId } from "src/redux/actions";
...
const SelectPaymentMethod = ({
	...
	const onSetupIntentResponse = async(response) => {
		...
		try {
			if (response.success) {
				const { data: { clientSecret, paymentId } } = response;
				...
				if (setupIntentError) {
					...
					setIsLoading(false);
					let data = { userId: endUserId, paymentId: setupIntent?.paymentMethodId };
					dispatchPaymentId(data, onNavigate);
				}
			} else {
				setIsLoading(false);
				...
				<ScrollView>
					<View style={styles.safeView}>
						<View style={styles.paymentInfoTitleContainer}>
							<Text style={styles.paymentInfoTitle}>{translate("payment.selectPaymentMethod")}</Text>
						</View>
...
const mapDispatchToProps = {
	...
	dispatchPaymentId: handlePaymentId,
};
